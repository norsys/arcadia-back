...
describe('/v1/questions', () => {
    ...
    describe('GET /v1/questions', () => {
        let questions = [{
            category_id: 1,
            question: 'Le projet qui te semble le plus interessant',
            response: 'toto',
            isEnable: true,
            type_id: 1
        }];
    ...
    describe('GET /v1/questions/agency/:agence_id', () => {
        let questions = [{
            id: 2,
            category_id: 1,
            question: 'Prendre une photo d\'un déjeuner en équipe'
        }];
        before('Insert seed data', () => helper.insertSeed(models['Question'], questions));
        after('Delete seed data', () => helper.deleteSeed(models['Question'], questions));
        ...
        it('should return 200 status code and an object', done => {
            request(app)
                .get(helper.bindAccessToken('/v1/questions/agency/2'))
                .expect(200)
                .end((err, res) => {
                    if (err) throw err;
                    res.body.should.be.instanceOf(Object);
                    res.body.inputType.should.be.equal("CAMERA");
                    done();
                });
        });
